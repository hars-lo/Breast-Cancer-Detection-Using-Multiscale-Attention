...
# --- 1. SINGLE-SCALE DATASET (The Fair Fight) ---
...
class BaselineResNet50(nn.Module):
    def __init__(self):
        ...
        # Using the standard modern weights parameter to avoid deprecation warnings
        self.model = models.resnet50(weights=models.ResNet50_Weights.DEFAULT)
        
        # Freeze the early layers so it doesn't unlearn basic image features
        for param in list(self.model.parameters())[:-30]: 
            param.requires_grad = False
            
        # Replace the final fully connected layer (which defaults to 1000 classes)
        # We replace it with a strong, regularized binary classification head
...
def main():
    ...
    # Create Results Directory for ResNet50
    ...
    # Using batch size 32 (Drop to 16 if your GPU runs out of memory)
    ...
    # Using a slightly lower learning rate because the backbone is pre-trained
    ...
    model_save_path = os.path.join(results_dir, "best_resnet50.pth")
    ...
    plt.legend(loc="lower right"); plt.title("ResNet50 - ROC Curve")
    plt.savefig(os.path.join(results_dir, "roc_curve.png"))

    print("\n=== ResNet50 Baseline Complete! ===")
